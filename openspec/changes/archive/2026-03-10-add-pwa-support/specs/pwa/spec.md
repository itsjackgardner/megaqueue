...
The application SHALL serve a web app manifest at `/static/manifest.json` that declares the app name as "MegaQueue", display mode as "standalone", theme color and background color matching the dark UI, start URL as "/", and icon references at 192x192 and 512x512 sizes in PNG format plus an SVG icon.
...
- **THEN** it contains `name`, `short_name`, `start_url`, `display`, `theme_color`, `background_color`, and `icons` fields
...
The application SHALL include app icons at `/static/icons/` in the following formats: `icon-192.png` (192x192), `icon-512.png` (512x512), and `icon.svg` (scalable).
