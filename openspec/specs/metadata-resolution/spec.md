...
The system SHALL run `guessit` on each `DownloadFile.name` as soon as the download manager reports it. Per-file results SHALL include at minimum `type` (`movie` or `episode`), `title`, and — where applicable — `year`, `season`, `episode`, `screen_size`, `source`, `video_codec`. The system SHALL NOT call any external metadata API (no TMDB, no TVDB, no network).

#### Scenario: Movie filename parsed
- **WHEN** the download manager reports `Birth.2004.Criterion.1080p.BluRay.x265.HEVC.FLAC-SARTRE.mkv`
- **THEN** guessit returns `type=movie`, `title="Birth"`, `year=2004`, `screen_size="1080p"`, `source="BluRay"`

#### Scenario: TV episode filename parsed
- **WHEN** the download manager reports `Gen.V.S02E06.1080p.10bit.WEBRip.6CH.x265.HEVC-PSA.mkv`
- **THEN** guessit returns `type=episode`, `title="Gen V"`, `season=2`, `episode=6`

#### Scenario: Bare extra filename parsed
- **WHEN** the download manager reports `Trailer.mkv`
...
- **WHEN** a Download has `metadata_source="user"` and a new file name is reported by the download manager
