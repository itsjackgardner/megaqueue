...
### Requirement: MegaDownloadManager tests
Tests SHALL verify the MegaDownloadManager handles status dict shape, start/cancel/remove operations, progress callback updates, proxy loading, and shutdown. The manager SHALL be tested with a real asyncio event loop in a daemon thread.

#### Scenario: Status returns correct shape
- **WHEN** `manager.status()` is called with no active downloads
- **THEN** it returns `{"running": True, "downloads": []}` 

#### Scenario: Start creates entries with correct fields
- **WHEN** `manager.start(["mega.nz/file/abc#key1"])` is called
- **THEN** the status dict contains an entry with `url`, `name`, `finished`, `bytesLoaded`, `bytesTotal`, `speed`, `error`, `sourceUrl` fields

#### Scenario: Cancel marks entry finished
- **WHEN** `manager.cancel(url)` is called for an active download
- **THEN** the entry's `finished` is True and `status` is "Cancelled"

### Requirement: Worker tests
Tests SHALL verify the worker's core logic: URL normalization, download-engine-to-database file matching, status derivation, and download state transitions. The MegaDownloadManager SHALL be mocked at the module level. Database operations SHALL use the in-memory database fixture.
...
- **WHEN** a Download has status "downloading" but the download manager reports no matching entries
...
Tests SHALL verify HTTP responses for key routes: dashboard (`GET /`), add download form (`GET /download/add`), create download (`POST /download`), download detail (`GET /download/<id>`), and status API (`GET /api/status`). The MegaDownloadManager SHALL be mocked.
...
#### Scenario: Create download queues files
- **WHEN** a POST is made to `/download` with two mega.nz URLs
- **THEN** a Download record is created with two DownloadFiles in QUEUED status (the worker submits them to the download manager on the next poll tick)
